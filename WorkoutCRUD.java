...
class WorkoutCRUD {
    ...
    public static ArrayList<Plan> PlanList = new ArrayList<>(); // PlanList 정의
    ...
    public void displayList() { // list of Exercises
        ...
        for (int i = 0; i < PlanList.get(this.index).getKind().length; i++) {
            if (!takingRest) {
                if (currentNum == i) {
                    System.out.println((i + 1) + ". " + PlanList.get(index).getKind()[i] + " (Performing)"); // 현재 차례가 온 운동이면서 수행 중
                } else {
                    System.out.println((i + 1) + ". " + PlanList.get(index).getKind()[i] );
                }
            } else {
                if (currentNum == i) {
                    System.out.println((i + 1) + ". " + PlanList.get(index).getKind()[i] + " (Taking Rest)"); // 다음 차례가 올 운동이고 휴식 중
                } else {
                    System.out.println((i + 1) + ". " + PlanList.get(index).getKind()[i]);
                }
            }
